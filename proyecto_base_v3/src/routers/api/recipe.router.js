...
export default class RecipeRouter extends BaseRouter {
    ...
    initialize() {
        ...
        this.addPutRoute("/:cid/ingredients/:pid", [STANDARD], (req, res) => this.#addOneIngredient(req, res));
        this.addDeleteRoute("/:cid/ingredients/:pid", [STANDARD], (req, res) => this.#removeOneIngredient(req, res));
        this.addDeleteRoute("/:cid/ingredients", [STANDARD], (req, res) => this.#removeAllIngredients(req, res));
    ...
    async #addOneIngredient(req, res) {
        try {
            ...
            const recipeUpdated = await this.#recipeManager.addOneIngredient(cid, pid, quantity ?? 1);
            res.sendSuccess200(recipeUpdated);
        } catch (error) {
            res.sendError(error);
        }
    ...
    async #removeOneIngredient(req, res) {
        try {
            const { cid, pid } = req.params;
            const recipeDeleted = await this.#recipeManager.removeOneIngredient(cid, pid, 1);
            res.sendSuccess200(recipeDeleted);
        } catch (error) {
            res.sendError(error);
        }
    ...
    async #removeAllIngredients(req, res) {
        try {
            const recipeDeleted = await this.#recipeManager.removeAllIngredients(req.params.cid);
            res.sendSuccess200(recipeDeleted);
        } catch (error) {
            res.sendError(error);
        }
