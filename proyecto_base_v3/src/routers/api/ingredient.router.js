...
export default class IngredientRouter extends BaseRouter {
    ...
    async #create(req, res) {
        try {
            const { file } = req;
            const ingredientCreated = await this.#ingredientManager.insertOne(req.body, file?.filename);
    ...
    async #update(req, res) {
        try {
            const { file } = req;
            const ingredientUpdated = await this.#ingredientManager.updateOneById(req.params.id, req.body, file?.filename);
